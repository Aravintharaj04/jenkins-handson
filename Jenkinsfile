        ...
        stage('Build and Push docker image') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'docker-credentials', usernameVariable: 'USER', passwordVariable: 'PASS')]){
                    sh "docker build -t aravintharaj04/nodejs-repo:${IMAGE_NAME} ."
                    sh 'echo $PASS | docker login -u $USER --password-stdin'
                    sh "docker push aravintharaj04/nodejs-repo:${IMAGE_NAME}"
                }
            }
        }
            ...
            steps {
                script {
                    withCredentials([usernamePassword(credentialsId: 'gitlab-credentials', usernameVariable: 'USER', passwordVariable: 'PASS')]) {
                        ...
                        sh 'git remote set-url origin https://$USER:$PASS@github.com/Aravintharaj04/jenkins-handson.git'
                        sh 'git add .'
